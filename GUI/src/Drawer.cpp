...
void Drawer::refresh(sf::TcpSocket &socket)
{
    ...
    //     std::string to_send = "ppo #" + std::to_string(player.getID()) + "\n";
    //     socket.send(to_send.c_str(), to_send.size());
    //     handleCommand(socket);
    //     to_send = "plv #" + std::to_string(player.getID()) + "\n";
    //     socket.send(to_send.c_str(), to_send.size());
    //     handleCommand(socket);
    // }
    if (_iDinventory != -1) {
...
{
    // create a 3D TEXT for each player. the 3d text is the msg and start at coordinates of the player and doo a parabole to the sky and then re-drop in parabol to coordiante of all other players
